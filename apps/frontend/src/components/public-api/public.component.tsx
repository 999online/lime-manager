...
const getMcpConfig = (
  ...
  if (method === 'path') {
    switch (client) {
      case 'Claude Code':
        return {
          config: `claude mcp add lime-manager --transport http "${urlWithKey}"`,
          hint: 'Run this command in your terminal.',
        };
      case 'Cursor':
        return {
          config: json({ mcpServers: { 'lime-manager': { url: urlWithKey } } }),
          hint: 'Add to .cursor/mcp.json in your project root.',
        };
      case 'VS Code / Copilot':
        return {
          config: json({
            servers: { 'lime-manager': { type: 'http', url: urlWithKey } },
          }),
      ...
      case 'Windsurf':
        return {
          config: json({
            mcpServers: { 'lime-manager': { serverUrl: urlWithKey } },
          }),
          hint: 'Add to ~/.codeium/windsurf/mcp_config.json',
        };
      case 'Amp':
        return {
          config: `amp mcp add lime-manager ${urlWithKey}`,
          hint: 'Run this command in your terminal.',
        };
      case 'Codex':
        return {
          config: `# ~/.codex/config.toml\n\n[mcp_servers.lime-manager]\nurl = "${urlWithKey}"`,
      ...
      case 'Warp':
        return {
          config: json({ 'lime-manager': { url: urlWithKey } }),
  ...
  switch (client) {
    case 'Claude Code':
      return {
        config: `claude mcp add --transport http lime-manager ${urlBase} --header "Authorization: ${bearer}"`,
        hint: 'Run this command in your terminal.',
      };
    case 'Cursor':
      return {
        config: json({
          mcpServers: {
            'lime-manager': { url: urlBase, headers: { Authorization: bearer } },
          },
        }),
        hint: 'Add to .cursor/mcp.json in your project root.',
      };
        ...
        <div>
          ...
          <div className="text-[13px] text-customColor18 mt-[2px]">
            {t(
              ...
              'Connect Lime Manager MCP server to your client (Http streaming) to schedule your posts faster.'
            )}
          </div>
        </div>
      </div>
...
const localCliSteps = [
  ...
  {
    label: 'Install the Lime Manager skill for your AI agent',
      ...
      <div className="bg-newBgColorInner px-[20px] py-[14px] border-b border-newBorder flex items-start justify-between gap-[12px]">
        <div>
          ...
          <div className="text-[13px] text-customColor18 mt-[2px]">
            {t(
              'cli_description',
              'Use the Lime Manager CLI to automate posting from your terminal, or install the skill to let your AI agent schedule posts for you.'
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col gap-[40px]">
      <div className="text-[14px] text-textColor leading-[1.7]">
        ...
        {t(
          'api_auth_note_line2',
          'If you are building a product that schedules posts on behalf of other Lime Manager users,'
        )}
        <br />
        {t(
          'api_auth_note_line3',
          ...
          <div>
            ...
            <div className="text-[13px] text-customColor18 mt-[2px]">
              {t(
                ...
                'Use Lime Manager API to integrate with your tools.'
              )}
            </div>
          </div>
        </div>
